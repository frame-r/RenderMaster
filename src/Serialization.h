...
public:
	...
	void print(IFile *file, int depth)
	{
		std::ostringstream oss;
...
typename std::enable_if<
	!std::is_base_of<Serializable<C>, T>::value &&
	!std::is_base_of<Serializable<C>, typename std::remove_pointer<T>::type>::value, void>::type
...
struct entry<C, tree<T>> : entry_base<C, tree<T>>
{
	...
	void print(void *p, IFile *file, int depth) const override
	{
		...
		while (it != tr.end())
		{
			...
			print_<C, T>(file, oss, it, tr.depth(it) + depth + 1);
			it++;
		}
