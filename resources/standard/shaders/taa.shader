...
float4 mainFS(VS_OUT fs_input, float4 screenPos : SV_Position) : SV_Target0
{
	...
	int2 uv = int2(screenPos.xy - vel * (data.xy ));
	float3 prev = texture_color_prev.Load(int3(uv, 0)).rgb;
	...
	prev = clamp(prev, color_min, color_max);

	float lum0 = luma(color);
	float lum1 = luma(prev);
	...
	float3 coolor_out = lerp(color, prev, 0.9);
